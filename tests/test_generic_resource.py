...
def create_dummy_crd(group="thisgroup", kind="thiskind", plural="thiskinds", scope="Namespaced", versions=None):
    if versions is None:
...
def test_signature_change_not_allowed():
    ...
    with pytest.raises(ValueError, match=r".*different signature"):
